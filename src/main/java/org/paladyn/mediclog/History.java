...
//          Log.d("mediclog","History - first line *"+line+"*");
        while ( line != null ) {
          ...
          if (values.length == 1 ) { sb. append("Something has gone wrong - line is "+line) ; historyTextView.setText(sb);
//                    Log.d("mediclog","History - something wrong *"+line+"*");
                    return; }
          if (values.length > 0) {sb.append(values[1]+"-"); }
          if (values.length> 4 ) { sb.append("("+values[2]+"," +values[3]+","+values[4]+") " ) ; }
          if (values.length> 5 ) { sb.append(values[5]+" "); }
          if (values.length> 6 ) { sb.append(values[6]); }

            if (values.length>7 && values[7].length() > 0 ) { sb.append("\n"+values[7]); }
