...
//    Log.d("mediclog","incHistBuffReadIndex -"+histBuffReadIndex);
    histBuffReadIndex++;
    if ( histBuffReadIndex > maxHistBuffLen) { histBuffReadIndex = 0;  /* Log.d("mediclog","Set histBuffReadIndex to zero");  */ }
    }
...
// if we have hit the number of records in the buffer, return null
    if (histBuffReadIndex  == histBuffMaxReadIndex ) { return null;  };
